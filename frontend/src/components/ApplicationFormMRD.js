...
function ApplicationForm() {
  ...
  const MIN_GPA = 9.2; // Example minimum GPA for eligibility
  const MAX_GPA = 10.0; // Example maximum GPA for eligibility
  ...
  const fetchStudentDetails = async (studentId) => {
    ...
    try {
      ...
      const formattedDob = new Date(student.dob).toISOString().split('T')[0];  // Format to YYYY-MM-DD
